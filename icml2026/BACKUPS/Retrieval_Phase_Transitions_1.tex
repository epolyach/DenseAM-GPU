...
\subsection{The Crosstalk and REM statistics}

In classical associative memory models, such as the Ising Hopfield network \cite{amit1987statistical}, the stability of a memory is threatened by the interference of the other stored patterns. In the regime where the number of memories scales linearly with the size of the network, $M = \alpha N$, this interference manifests itself as local noise. For binary neurons, this crosstalk behaves as a Gaussian field with variance proportional to $\alpha$, leading to the standard replica-symmetric free energy. Within our macroscopic framework, this interference is captured by the term $a(M, q)$ in the total free energy density:
\begin{equation}
    f(\phi, q) = u(\phi) - T s(\phi, q) + a(M, q).
    \label{eq:total_free_energy}
\end{equation}
For the $k=2$ (quadratic) case with linear load, this term is given by 
\begin{equation}
    a_{\text{AGS}}(\alpha, q) = -\frac{\alpha}{2\beta} \ln [1 - \beta(1 - q)]
\end{equation}
Physically, this term acts as a repulsive potential; it prevents the ``thermal cloud'' from becoming too tight (i.e., prevents $q \to 1$ too quickly), eventually causing the retrieval phase to destabilize and ``melt'' when the load $\alpha$ becomes too high.

However, the modern continuous DAMs considered here are designed to operate in the exponential capacity regime, where $M = e^{\alpha N}$. In this limit, the local Gaussian approximation for crosstalk used in $k=2$ Hamiltonians is no longer sufficient. When the number of patterns is exponential, the system is not merely perturbed by local noise; rather, the retrieval branch must compete with an exponentially large number of spurious states. This disordered landscape is best described by the statistics of the Random Energy Model (REM) \cite{derrida1980random}.

The REM refinement supersedes the classical crosstalk term by treating the energies of the $M-1$ non-retrieved patterns as independent random variables. Because $M$ is exponential in $N$, there is a non-negligible probability that a random configuration $\mathbf{x}$, uncorrelated with the target pattern, will almost always find itself accidentally close to some pattern or a combination of patterns. This accidental alignment creates a low-energy state (the noise floor) that competes with the actual intended retrieval state.

To use these statistics in Eq.~(\ref{eq:total_free_energy}), we identify $a(M, q)$ as the free energy contribution of the ``melted'' or disordered branch. In this branch, the overlaps $\phi^\mu$ are Gaussian variables $\mathcal{N}(0, 1/N)$, and the resulting energy density $u_{\text{melt}}$ is determined by the extreme value statistics of the sum of $M$ exponential terms. This term provides the ``noise floor'' of the system. Retrieval remains thermodynamically stable only as long as the localized free energy of the retrieval well (where $\phi > 0$) remains lower than this REM-governed noise floor. 

\subsection{The Melted Branch and REM Statistics}

In the exponential capacity regime ($M = e^{\alpha N}$), the thermodynamic stability of the network is not determined by local fluctuations of the crosstalk, but by a global competition between two distinct branches of the partition function: the \textit{retrieval branch} and the \textit{melted branch}. In the retrieval branch, the system is localized near a single target pattern ($\phi \approx 1$). Its free energy density is given by:
\begin{equation}
    f_{\rm ret}(\phi, q) = u(\phi) - T s(\phi, q),
\end{equation}
where the crosstalk term $a(M, q)$ is effectively zero because the sharply peaked kernels (LSE and LSR) exponentially suppress the contributions of all non-target patterns.

However, as the memory load $\alpha$ or the temperature $T$ increases, this retrieval state must compete with the ``melted'' branch that represents a disordered state where the system is not aligned with any specific memory ($\phi = 0$). In this branch, the energy is determined by the collective interference of the $M$ patterns. To find the energy density of this branch, $u_{\rm melt}$, we utilize the statistics of the Random Energy Model (REM) \cite{derrida1980random}. 
...
\[
g(\phi) = \alpha - \beta_{\rm net}(1 - \phi) - \phi^2/2\,.
\]
...
\begin{equation}
    ...
    \label{eq:alpha_th}
\end{equation}
If $\alpha < \alpha_{\rm th}$, the expected number of random patterns within the support of any state is effectively zero, and a disordered melted branch cannot form. For $\alpha > \alpha_{\rm th}$, the noise floor is determined by the logarithmic energy density $u_{\rm LSR}(\phi) = -\hat\beta_{\rm net}^{-1} \ln ( 1 - \hat\beta_{\rm net}(1 - \phi) )$.

The partition function of the noise branch is determined by the integral over the patterns within support:
\begin{multline}
    p_{\rm noise} = \sum_{\mu=1}^{M} \left( 1 - \hat\beta_{\rm net} (1 - \phi^\mu) \right) \approx \\
    \int_{1-1/\hat\beta_{\rm net}}^{\phi_{\rm max}} \dd\phi \exp\left( N \left[ \alpha - \frac{\phi^2}{2} \right] \right) \left( 1 - \hat\beta_{\rm net} (1 - \phi) \right),
\end{multline}

\begin{equation}
    \beta_{\rm net} = f(\phi^*) = \frac{\phi^*}{1 + \phi^* - (\phi^*)^2}\,,
\end{equation}
(note that $f(\phi^*)$ is monotonically growing in the interval $[0,1]$).
...
\subsubsection{Retrieval Stability Condition}

Because the LSE and LSR energy wells are extremely sharp, the transition from retrieval to the disordered state is a first-order jump (melting). The retrieval phase is thermodynamically stable if and only if its free energy is lower than the energy of the melted branch:
\begin{equation}
    \min_{\phi, q} \left[ u(\phi) - T s(\phi, q) \right] \leq u_{\rm melt}.
\end{equation}
As $T$ increases, the geometric entropy $s$ grows, pushing the retrieval free energy up. Simultaneously, as the load $\alpha$ increases, the noise floor $u_{\rm melt}$ drops. The transition occurs at the critical temperature $T_c$ where these two values cross. Beyond this point, the system instantaneously "boils" out of the retrieval well and into the disordered state.


\subsection{The Crosstalk and REM statistics}

The thermodynamic stability of retrieval in associative memory is fundamentally dictated by the interference between stored patterns, or "crosstalk." In the classical Ising Hopfield model \cite{amit1987statistical}, where the number of memories scales linearly with the number of neurons, this interference manifests as a local Gaussian noise. Within the macroscopic replica-symmetric framework, this interference is captured by an additional term $a(M, q)$ in the total free energy density, as expressed in Eq.~\eqref{eq:total_free_energy}:
\begin{equation}
    f(\phi, q) = u(\phi) - T s(\phi, q) + a(M, q),
    \label{eq:total_free_energy}
\end{equation}
where we have updated the notation for the total number of memories from $p$ to $M$. For linear loads ($M = \alpha N$), the term $a(M, q)$ acts as a repulsive potential that prevents the "thermal cloud" from becoming too tight, eventually causing the retrieval phase to destabilize once the internal noise exceeds the energetic depth of the retrieval well.

However, in modern Dense Associative Memories (DAMs) operating in the exponential capacity regime ($M = e^{\alpha N}$), the crosstalk is no longer a mere local perturbation. Instead, the retrieval branch must compete with an exponentially large "sea" of spurious patterns. In this regime, the interference term $a(M, q)$ is superseded by the global noise floor of the system, which is determined by the statistics of the Random Energy Model (REM) \cite{derrida1980random}. In these models, the retrieval wells are so sharp that non-target patterns do not contribute to the local curvature of the well ($a \to 0$); instead, the system "boils" out of retrieval and into a disordered state when the free energy of the retrieval branch exceeds the energy of this collective noise floor, denoted as $u_{\rm melt}$.
