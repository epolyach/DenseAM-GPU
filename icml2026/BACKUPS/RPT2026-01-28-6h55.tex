...
Dense Associative Memory (DAM) networks extend classical Hopfield models by replacing pairwise interactions with highly nonlinear energy functions to significantly improve storage capacity and robustness. When defined over continuous states constrained to an $N$-dimensional sphere, DAMs can store an exponential number of patterns ($p = \exp(\alpha N)$) with perfect recall in the zero-temperature limit \citep{demircigil2017model,ramsauer2021hopfield,lucibello2024exponential}. These models have garnered renewed interest due to their formal equivalence to softmax attention mechanisms in Transformers, where energy-based updates closely mirror attention-driven memory retrieval \citep{ramsauer2021hopfield}. However, existing theoretical results primarily describe behavior at zero temperature, leaving open the question of how memory stability persists under noise and thermal fluctuations.

Recent studies have begun to revisit associative memory from a thermodynamic perspective. Building on the classical theory of retrieval-to-spin-glass transitions in Hopfield networks \citep{amit1987statistical}, \citet{rooke2026stochastic} introduced a stochastic thermodynamics framework for DAMs, quantifying entropy production during memory retrieval and highlighting trade-offs between energy efficiency and recall performance. Parallel work by \citet{lucibello2024exponential} analyzed retrieval stability in DAMs with exponential capacity, emphasizing the role of entropic penalties in high-dimensional settings. These developments motivate a deeper understanding of how thermal noise affects the retrieval phase in continuous, high-capacity associative memory systems.

In this work, we analytically characterize the thermodynamic retrieval phase of continuous DAMs under finite temperature and exponential memory load. Specifically, we derive the phase boundary $\alpha_c(T)$ in the $(\alpha, T)$ plane that separates successful retrieval from disordered behavior. We consider DAMs defined on the $N$-sphere and study two kernel-based energy functions: the Gaussian log-sum-exp (LSE) and the compactly supported log-sum-ReLU (LSR) \citep{hoover2025dense}. While both achieve exponential capacity in the zero-temperature limit, they differ fundamentally in how kernel sharpness mediates robustness to noise: LSE yields smooth energy basins that degrade gradually, whereas LSR enforces sharper alignment at the cost of reduced thermal tolerance.

We show that this trade-off arises from the interplay between energy minimization and geometric entropy in high-dimensional space. As the number of stored patterns increases, their angular separation on the sphere decreases, and sharper kernels amplify the entropic penalty for achieving high alignment. As a result, LSR exhibits a finite critical temperature beyond which retrieval fails, whereas LSE permits memory recovery at arbitrarily high temperatures under sufficiently low load.
...
\section{Equilibrium Analysis and Critical Lines}
