...
\begin{equation}
    ...
    \label{eq:dens}
\end{equation}
This form corresponds to the negative log-likelihood of a Gaussian kernel density estimate of the memory distribution~\citep{hoover2025dense}. Under the $N$-sphere constraint, the dot product and distance formulations are equivalent:
\begin{equation}
...
\begin{equation}
    ...
    \approx  u(\phi) - T s\,,
\end{equation}
where $s$ is the entropy density and $u(\phi)$ is the internal energy density of a single memory basin. For the LSE and LSR kernels (see Appendices~A and~B):
\begin{equation}
    u_{\rm LSE} = 1-\phi\,,
    \label{eq:u_lse}
\end{equation}
...
This expression holds for a single microstate with alignment $\phi$. In the thermal setting, under the replica symmetry ansatz, the system occupies a cloud of states centered at a point with alignment $\phi$ to the target pattern. We characterize this cloud by the self-overlap parameter:
\begin{equation}
...
\section{Retrieval–Spin-Glass Transition}

Here we focus only on the transition between retrieval and spin-glass phases. Before deriving phase boundaries, we clarify the optimization problem that determines the network's steady state.
...
\begin{figure}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\columnwidth]{fig_phase_diagram.eps}}
...
\begin{figure*}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\textwidth]{fig_equilibrium.eps}}
...
In the retrieval basin of $\bm{\xi}$, the density $p(\bm{x}, \bm{\xi})$~\eqref{eq:dens} is dominated by the target memory:
\begin{equation}
    p(\bm{x}, \bm{\xi}) \approx \exp\left[ - \frac{\beta_{\rm net}}{2} d^2(\bm{x}, \bm{\xi}) \right] \,.
    \label{eq:dens_red}    
\end{equation}
