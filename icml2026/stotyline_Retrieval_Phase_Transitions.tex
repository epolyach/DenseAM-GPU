...
\begin{abstract}
\textbf{Dense Associative Memory (DAM) networks are a theoretical model underlying modern Hopfield networks and attention mechanisms, capable of storing an exponential number of patterns. While their zero-temperature capacity is well understood, much less is known about their stability under noise.}
We derive thermodynamic phase boundaries for Dense Associative Memory networks with exponential capacity $P = e^{\alpha N}$, comparing Gaussian (LSE) and Epanechnikov (LSR) kernels. 
\textbf{We show that this stability is controlled by a universal geometric entropy induced by the spherical constraint, which competes with kernel-dependent energy.} For continuous neurons on an $N$-sphere, the geometric entropy depends solely on the spherical geometry, not the kernel. Sufficiently sharp kernels achieve maximum theoretical capacity $\alpha = 0.5$ at zero temperature; below this threshold, a critical line separates retrieval from a spin-glass phase. Kernel sharpness introduces a trade-off between energetic stability and entropic cost: while LSR provides deeper energy minima, it exhibits a smaller retrieval region than LSE at finite temperature because enforcing higher alignment incurs a geometric entropy penalty that outweighs the energetic gain. The kernels also differ qualitatively in their thermal limits: LSR possesses a finite maximum retrieval temperature, whereas LSE permits retrieval at arbitrarily high temperatures for sufficiently low loads.
...
Associative memory networks store patterns such that the correct memory can be retrieved from a noisy or partial cue. The classic example is the Hopfield network~\citep{Hopfield1982}, which can reliably retrieve a memory pattern given a cue, up to a certain limit on the number of patterns stored. In the original Hopfield model, each neuron is binary and the network's memory capacity scales linearly with the number of neurons $N$ (on the order of $0.14N$ patterns for low-error retrieval in the symmetric binary case as shown by~\citet{amit1985storing}). Over the years, many extensions have been proposed to achieve super-linear capacity. For instance, fully connected networks with $p$-body (higher-order) interactions can yield polynomial scaling of capacity~\citep{gardner1988space,KrotovHopfield2016}. More recently, formulations with exponential capacity have been developed~\citep{demircigil2017model,ramsauer2021hopfield}, known as Dense Associative Memory (DAM) or modern Hopfield networks, where the number of storable patterns $P$ scales as $P = e^{\alpha N}$.

These models have gained renewed interest in machine learning due to their connections with attention mechanisms in Transformers. Specifically, the energy-based updates in continuous DAMs correspond to attention lookups via the log-sum-exp operation, providing a principled energy-based view of memory retrieval~\citep{ramsauer2021hopfield}.

\textbf{In the context of machine learning, temperature can be interpreted as modeling uncertainty, stochasticity, or limited numerical precision in memory retrieval and attention mechanisms. Understanding how retrieval degrades as temperature increases therefore provides insight into the robustness of attention-like models under noise.}

Despite their promise, theoretical understanding of DAMs remains incomplete, especially in noisy or uncertain settings. Most existing results describe the zero-temperature limit, where retrieval corresponds to strict energy minimization. Yet in realistic applications, memory access occurs under noise, uncertainty, or limited precision, concepts naturally modeled as finite temperature in a thermodynamic framework.

Recent work has begun to address this gap. Building on classic phase transition analyses in Hopfield models~\citep{amit1987statistical}, \citet{rooke2026stochastic} introduced a stochastic thermodynamics framework for DAMs, quantifying entropy production and trade-offs between energy cost and retrieval fidelity. In parallel, \citet{lucibello2024exponential} analyzed exponential-capacity DAMs using a Random Energy Model (REM), emphasizing entropic penalties at high dimension and deriving phase transition criteria.

\textbf{From a machine learning perspective, the key question is not only how many patterns can be stored, but under what conditions retrieval remains stable when noise and interference are present.}
Our work continues and extends this line of inquiry. We derive finite-temperature phase boundaries $\alpha_c(T)$ for continuous DAMs on the $N$-sphere under exponential memory load. We reveal that retrieval stability is determined by a competition between a kernel-dependent energy term and a universal, kernel-independent \emph{geometric entropy} induced by the spherical constraint.

To demonstrate this, we analyze two representative DAM energy functions: the Gaussian log-sum-exp (LSE) and the compactly supported log-sum-ReLU (LSR)~\citep{hoover2025dense}. Both attain exponential capacity as $T \to 0$, but exhibit qualitatively distinct thermal behaviors. LSR yields sharper energy wells but suffers from a stronger entropy penalty, resulting in a finite critical retrieval temperature. In contrast, LSE allows retrieval at arbitrarily high temperatures under low memory load.

In high dimensions, as the number of stored patterns increases, their typical angular separation decreases. This makes it entropically more costly to maintain strong alignment with a target pattern, especially when the energy kernel is sharp. Consequently, while the LSR kernel provides deeper energy wells, it suffers from a stronger entropic penalty and supports retrieval only below a finite critical temperature. In contrast, the smoother LSE kernel tolerates higher thermal fluctuations and permits retrieval at arbitrarily high temperatures, provided the memory load is sufficiently low.

Our main contributions are:
\begin{enumerate}
\item Analytical characterization of the finite-temperature retrieval transition in continuous DAMs with exponential memory capacity, including the derivation of the critical phase boundary $\alpha_c(T)$ on the $N$-sphere.
\item Identification of a universal geometric entropy term induced by the spherical constraint, and analysis of its interplay with kernel-dependent energy in determining retrieval stability.
\item Derivation of explicit phase diagrams for Gaussian (LSE) and Epanechnikov (LSR) kernels, revealing a finite maximal retrieval temperature for LSR and unbounded thermal stability for LSE at low load.
\end{enumerate}

These results extend thermodynamic theories of associative memory to the finite-temperature regime, elucidating which features of retrieval stability stem from geometry and which are determined by the choice of kernel. Our analysis provides a foundation for designing robust, high-capacity memory systems in machine learning models that operate under structured data and stochastic dynamics.
...
\textbf{This section introduces Dense Associative Memory (DAM) models from the perspective of energy-based learning. The key idea is that memory retrieval corresponds to minimizing an energy function whose landscape is shaped by stored patterns.}

\textbf{We begin by recalling two classes of energy functions (Hamiltonians) that have been proposed to define associative memory dynamics.}
The first type is $k$-polynomial~\cite{krotov2016dense}, 
\begin{equation}
...
\textbf{The LSE energy is particularly important for machine learning, as its gradient-based update rule is mathematically equivalent to the softmax attention mechanism used in Transformers.}
...
\begin{equation}
    ...
    \label{eq:dens}
\end{equation}
This form admits a statistical interpretation: the energy corresponds to the negative log-likelihood of a Kernel Density Estimate (KDE) of the memory distribution with a Gaussian kernel~\cite{hoover2025dense}. 
\textbf{The LSR formulation replaces the exponential kernel with a compactly supported one, which can be interpreted as enforcing harder locality in memory access.}

Under the $N$-sphere constraint, however, these two formulations are  equivalent, since 
\begin{equation}
...
\textbf{To analyze retrieval at finite temperature and exponential load, we adopt tools from statistical mechanics. While some of these techniques (e.g., the replica method) originate from physics, they serve here as analytical devices for computing typical-case behavior of high-dimensional energy-based models.}
...
\textbf{The replica method provides a way to compute the typical free energy by averaging over random memory patterns, which play the role of quenched disorder.}
...
\textbf{The central object in our analysis is the geometric entropy: the logarithmic volume of configurations on the $N$-sphere that maintain a given alignment with a memory pattern. This term is universal in the sense that it depends only on geometry, not on the choice of kernel.}
...
\textbf{
This entropy quantifies how restrictive it is, in high dimension, to maintain strong alignment with a specific pattern.}

\subsection{Crosstalk and the Melted Branch}

\textbf{From a machine learning viewpoint, the melted branch corresponds to a failure mode where retrieval is dominated by accidental correlations with many stored patterns rather than the intended target.}
...
\textbf{This section plays the role of an analytical experiment, characterizing equilibrium behavior and phase boundaries.}
...
For the Epanechnikov (LSR) kernel, the energy density is:
\begin{equation}
    u_{\rm LSR}(\phi) = -\frac{1}{b} \ln\left(1 - b(1-\phi)\right),
\end{equation}
where $b \equiv \hat{\beta}_{\rm net} = N\beta_{\rm net}$ is the rescaled network sharpness. The retrieval free energy is:
\begin{equation}
    f_{\rm ret}(\phi, q) = -\frac{1}{b} \ln\left(1 - b(1-\phi)\right) - T s(\phi, q).
\end{equation}
...
\end{equation}
For $b = 1$, the solution simplifies to $\phi_{\rm LSR}(T) = 1/\sqrt{1+T}$.

\paragraph{Support Threshold and Capacity.} The parameter $b$ controls both the support boundary $\phi > 1 - 1/b$ and the support threshold:
\begin{equation}
    \alpha_{\rm th}(b) = \frac{1}{2}\left(1 - \frac{1}{b}\right)^2.
\end{equation}
For $\alpha < \alpha_{\rm th}$, no spurious patterns fall within the kernel's support, providing enhanced retrieval stability. Table~\ref{tab:lsr_b_values} summarizes key properties for three representative values of $b$:
\begin{itemize}
    \item $b = 1$: Degenerate case with $\alpha_{\rm th} = 0$ (no compact support effect).
    \item $b = 2 + \sqrt{2} \approx 3.41$: Gives $\alpha_{\rm th} = 0.25$, half of the maximum capacity.
    \item $b = 4 + 2\sqrt{3} \approx 7.46$: Gives $\alpha_{\rm th} = 0.375$, three-quarters of maximum.
\end{itemize}

\paragraph{Critical Line.} For LSR, the critical line must account for the support threshold:
\begin{equation}
    \boxed{\alpha_c^{\rm LSR}(T; b) = \max\left(\alpha_{\rm th}(b),\; \frac{(1 - f_{\rm ret}^{\rm LSR}(T; b))^2}{2}\right)}
    \label{eq:alpha_lsr}
\end{equation}

\paragraph{Maximum Temperature.} Unlike LSE, the LSR free energy grows without bound as $T \to \infty$. Retrieval requires $f_{\rm ret} < 1$ (since $u_{\rm melt} \le 1$). This yields a finite maximum temperature $T_{\max}(b)$ that \emph{decreases} with increasing $b$: sharper kernels enforce higher alignment, incurring larger entropy penalties. Above $T_{\max}$, retrieval is impossible at any memory load---a qualitative difference from LSE.

\paragraph{Remark: The sub-threshold regime $\alpha < \alpha_{\rm th}$.} For $b > 1$, the critical line terminates at $(\alpha_{\rm th}, T_{\max})$, not at $(0, T_{\max})$. This reflects a qualitative change in the physics below the support threshold. When $\alpha < \alpha_{\rm th}$, no spurious patterns fall within the kernel's support region $\phi > 1 - 1/b$, so the REM-derived noise floor $u_{\rm melt}$ does not apply. The retrieval basin is completely isolated from interference. In this regime, the retrieval-to-disorder transition is not driven by competition with a melted branch, but potentially by escape to states outside all pattern supports---which for LSR have divergent energy. A full analysis of thermal stability in the sub-threshold regime requires different methods and is left for future work. The horizontal segment of the critical line at $\alpha = \alpha_{\rm th}$ in Figure~\ref{fig:phase_diagram} marks the boundary of the region where our REM-based analysis applies.

\subsection{Comparison: Energy vs Entropy Trade-off}

\begin{table}[t]
\caption{Summary of LSR model properties for different values of $b = \hat{\beta}_{\rm net}$. For $b > 1$, $T_{\max}$ is achieved at $\alpha = \alpha_{\rm th}$; see Remark in Section~4.2.}
\label{tab:lsr_b_values}
\vskip 0.1in
\begin{center}
\begin{small}
\begin{tabular}{lcccc}
\toprule
Model & Support & $\alpha_{\rm th}$ & $T_{\max}$ & $\phi(T\!=\!1)$ \\
\midrule
LSE (Gaussian) & $\phi > 0$ & 0.5 & $\infty$ & 0.618 \\
LSR ($b = 1$) & $\phi > 0$ & 0 & 2.24 & 0.707 \\
LSR ($b = 3.41$) & $\phi > 0.71$ & 0.25 & 1.31 & 0.775 \\
LSR ($b = 7.46$) & $\phi > 0.87$ & 0.375 & 0.80 & 0.833 \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
Table~\ref{tab:lsr_b_values} reveals the fundamental trade-off in LSR models: increasing $b$ raises the support threshold $\alpha_{\rm th}$, providing immunity to spurious patterns at low loads, but simultaneously \emph{decreases} the maximum retrieval temperature $T_{\max}$. This occurs because larger $b$ enforces higher equilibrium alignment $\phi$, which incurs a larger geometric entropy penalty $s(\phi) = \frac{1}{2}\ln(1-\phi^2)$. The entropy cost $-Ts$ grows faster than the energy benefit, causing the retrieval free energy to cross the melting threshold at lower temperatures.

Compared to LSE, all LSR variants have smaller retrieval regions at finite temperature despite their deeper energy wells. The LSR kernel forces higher alignment $\phi$, achieving lower energy $u$. However, higher $\phi$ means the configuration is more constrained, leading to more negative entropy $s$. The entropy penalty outweighs the energy gain, resulting in higher free energy and earlier melting.

\subsection{Phase Diagrams}

Figure~\ref{fig:phase_diagram} shows the critical lines for LSE and LSR (with three values of $b$) in the $(\alpha, T)$ plane. Key features:
\begin{itemize}
    \item All kernels achieve $\alpha_c(0) = 0.5$ at zero temperature.
    \item LSE (solid black) permits retrieval at arbitrarily high $T$ for small $\alpha$.
    \item LSR critical lines terminate at finite $T_{\max}(b)$, which decreases with $b$.
    \item For $b > 1$, LSR has a support threshold $\alpha_{\rm th}(b)$ (marked by squares) below which the melted branch cannot form.
    \item At any finite $T$, all LSR variants have smaller $\alpha_c$ than LSE.
\end{itemize}

\begin{figure}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\columnwidth]{fig_phase_diagram_3.eps}}
    \caption{Phase diagram for spherical DAM with exponential capacity $p = e^{\alpha N}$. The retrieval region (shaded blue) is bounded by the critical lines $\alpha_c(T)$. LSE (solid black) permits retrieval at arbitrarily high temperatures. LSR curves for $b = 1, 3.41, 7.46$ exhibit decreasing maximum temperatures $T_{\max}$ and increasing support thresholds $\alpha_{\rm th}$ (squares on $\alpha$-axis). The boundary at $\alpha = 0.5$ separates spin-glass from paramagnetic disordered phases.}
...
\begin{figure*}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\textwidth]{fig_equilibrium_3.eps}}
    \caption{Equilibrium properties vs temperature for LSE and LSR with $b = 1, 3.41, 7.46$. \textit{Left:} Alignment $\phi(T)$---larger $b$ enforces higher alignment (horizontal dashed lines show support boundaries $\phi = 1 - 1/b$). \textit{Right:} Retrieval free energy $f_{\rm ret}(T)$---larger $b$ causes faster growth, crossing the melting threshold $f = 1$ at lower $T_{\max}$ (circles).}
    \label{fig:equilibrium}
  \end{center}
  \vskip -0.1in
\end{figure*}

Figure~\ref{fig:equilibrium} compares the equilibrium alignment and free energy for all models. Larger $b$ maintains higher $\phi(T)$ at all temperatures due to sharper energy wells, but this ``tightness'' incurs a larger entropy penalty. The trade-off is stark: $b = 7.46$ achieves $\alpha_{\rm th} = 0.375$ (75\% of maximum capacity immune to spurious patterns) but can only retrieve at $T < 0.80$, while $b = 1$ has no support threshold but permits retrieval up to $T \approx 2.24$.
...
\textbf{More broadly, our results suggest that robustness of attention-like mechanisms is fundamentally constrained by geometry, and that increasing sharpness or selectivity can reduce tolerance to noise due to entropic effects.}
