...
\begin{equation}
	...
	\label{eq:entropy_g}
\end{equation}
This expression holds for a single microstate with alignment $\phi$. This dependence is purely geometric and does not involve any kernel-specific assumptions.
...
\begin{enumerate}
    \item \textbf{Spin-glass regime} ($\beta_{\rm net} > \sqrt{2\alpha}$): The saddle point lies beyond $\phi_{\rm max}$, so the integral is dominated by the maximum spurious alignment:
    \begin{equation}
        u_{\rm noise} = 1 - \sqrt{2\alpha}.
    \end{equation}
    
    \item \textbf{Paramagnetic regime} ($\beta_{\rm net} < \sqrt{2\alpha}$): The saddle point $\phi^* = \beta_{\rm net}$ lies within the available range, giving:
    \begin{equation}
...
\begin{enumerate}
    \item \textbf{Spin-glass regime} ($b > f(\phi_{\rm max})$): The saddle point lies beyond $\phi_{\rm max}$, so the integral is dominated by the maximum spurious alignment:
    \begin{equation}
        u_{\rm noise} = -b^{-1} \ln \left( 1 - b(1 - \sqrt{2\alpha}) \right).
    \end{equation}
    Unlike LSE, this energy diverges as $\sqrt{2\alpha} \to 1 - 1/b$.
    
    \item \textbf{Paramagnetic regime} ($b < f(\phi_{\rm max})$): The saddle point lies within the available range:
    \begin{equation}
...
\section{Retrieval--Spin-Glass Transition}\label{sec:transition}
...
We focus only on the transition between retrieval and spin-glass phases. Before deriving phase boundaries, we clarify the optimization problem that determines the network's steady state. 
...
\subsection{LSE Spin-Glass Transition}
...
\begin{equation}
    ...
    \label{eq:f_lse}
\end{equation}
The phase boundary occurs when equality holds in~\eqref{eq:stability}. In the spin-glass regime:
\begin{equation}
    \alpha_c^{\rm LSE}(T) = \frac{1}{2} \Big[1 - f_{\rm ret}^{\rm LSE}(T) \Big]^2\,.
    \label{eq:alpha_lse}
\end{equation}
As $T \to \infty$, $\alpha_c \to 0$, so LSE permits retrieval at arbitrarily high temperatures for sufficiently small $\alpha$. In the limit $T \to 0$, one recovers the well-known result $\alpha_c = 0.5$ by~\citet{ramsauer2021hopfield}.


\subsection{LSR Spin-Glass Transition}
...
\end{equation}
For $\alpha < \alpha_{\rm th}$, no spurious patterns fall within the kernel's support, providing enhanced retrieval stability. Above the threshold, the phase boundary with the spin-glass regime is:
\begin{equation}
...
\begin{figure*}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=0.9\textwidth]{fig_phase_diagram_5.eps}}
    \caption{Phase diagrams for spherical DAM with exponential capacity $p = e^{\alpha N}$. \textbf{Left:} LSE kernel. The retrieval region (blue) permits retrieval at arbitrarily high temperatures for sufficiently low $\alpha$. \textbf{Right:} LSR kernel with $b = 3.41$. The retrieval region exhibits a support threshold $\alpha_{\text{th}} = 0.25$. Below this threshold, retrieval is perfect at any temperature. Above threshold, the critical line $\alpha_c(T)$ bounds the retrieval region, reaching $T_{\max} \approx 0.22$ at $\alpha = \alpha_{\text{th}}$. Both kernels achieve $\alpha_c(0) = 0.5$ at zero temperature. The vertical boundary at $\alpha = 0.5$ separates the spin-glass phase from the paramagnetic phase.}
    \label{fig:phase_diagram}
  \end{center}
  \vskip -0.1in
\end{figure*}

Fig.\,\ref{fig:phase_diagram} shows the phase diagrams in the $(\alpha, T)$ plane for LSE (left) and LSR (right) kernels. For LSR, we choose $b = 3.41$, which yields a support threshold $\alpha_{\text{th}} = 0.25$ below which retrieval is perfect at any temperature. This value of $b$ illustrates the qualitative advantage of finite-support kernels while maintaining a substantial retrieval region above threshold. The retrieval region (shaded) corresponds to parameter values where the system maintains macroscopic alignment with a target pattern. Three distinct phases emerge:

\begin{enumerate}
    \item \textbf{Retrieval phase} (low $\alpha$, low $T$):  The system converges to a state with high alignment $\phi \approx 1$ with the target memory. For LSE, this phase is bounded from above by the critical line $\alpha_c(T)$ at all temperatures. For LSR with $b > 1$, retrieval below the support threshold $\alpha_{\text{th}}$ is perfect at any temperature, as no spurious patterns fall within the kernel's support.
    
    \item \textbf{Spin-glass phase} (high $\alpha$, low $T$): When the memory load exceeds the critical value, interference from exponentially many stored patterns destabilizes retrieval. The system becomes trapped in disordered states with no macroscopic alignment to any single pattern.
    
    \item \textbf{Paramagnetic phase} (high $T$): Thermal fluctuations dominate, and the system explores the sphere uniformly without preference for any memory.
\end{enumerate}

\begin{figure*}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=0.9\textwidth]{mc_validation_icml.eps}}
    \caption{Equilibrium properties versus temperature. \textbf{Left:} Alignment $\phi(T)$ from theory (curves) and Monte Carlo simulation (symbols, $N = 50$, $\alpha = 0.1$). For LSR with $b = 3.41$, the system remains in the retrieval state across the temperature range shown, consistent with the phase boundary in Fig.~1 (right). For LSE, alignment drops sharply around $T \approx 0.7$--$0.8$, reflecting the phase transition at this load. \textbf{Right:} Retrieval free energy $f_{\text{ret}}(T)$ for LSE and LSR ($b = 3.41$).}
    \label{fig:equilibrium}
  \end{center}
  \vskip -0.1in
\end{figure*}

The critical lines $\alpha_c(T)$ differ qualitatively between kernels. For LSE (solid curve), the retrieval region extends to arbitrarily high temperatures as $\alpha \to 0$. For LSR with $b = 3.41$ (right panel), the critical line terminates at the support threshold $\alpha_{\text{th}} = 0.25$. Below this threshold, the retrieval basin is completely isolated from spurious pattern interference, and retrieval is perfect at any temperature.

Figure~\ref{fig:equilibrium} compares equilibrium properties as functions of temperature. The left panel shows the equilibrium alignment $\phi(T)$ obtained by minimizing the retrieval free energy, along with Monte Carlo validation (symbols, $N = 50$, $\alpha = 0.1$). For LSR with $b = 3.41$, the load $\alpha = 0.1$ lies below the support threshold $\alpha_{\text{th}} = 0.25$, and accordingly the system remains in the retrieval state across the full temperature range shown. For LSE, the alignment drops sharply around $T \approx 0.7$--$0.8$, reflecting the retrieval-to-disorder transition at this load. The right panel shows the corresponding retrieval free energies $f_{\text{ret}}(T)$.

\paragraph{Monte Carlo validation.}
These simulations provide a qualitative validation of the finite-temperature trends at low memory load, rather than a quantitative finite-size analysis.
To verify the theoretical predictions, we performed Metropolis--Hastings simulations on networks of size $N = 50$ with memory load $\alpha = 0.1$. For LSR with $b = 3.41$, this load lies below the support threshold $\alpha_{\mathrm{th}} = 0.25$, allowing us to test the predicted regime of perfect retrieval at any temperature. For LSE at the same load, the system should exhibit a retrieval-to-disorder transition at finite temperature. Patterns were sampled uniformly on the $N$-sphere, and the system was initialized near the target pattern. At each temperature, we equilibrated for 1500 steps followed by 1000 sampling steps, measuring the alignment $\phi = N^{-1} \mathbf{x} \cdot \boldsymbol{\xi}^1$ with the target.

The Monte Carlo results (symbols in Figure~\ref{fig:equilibrium}, left panel) show good agreement with theory. For LSE, the measured alignment follows the theoretical curve (\ref{eq:phi_lse}) and then drops sharply around $T \approx 0.7$--$0.8$, reflecting the retrieval-to-disorder transition where interference from spurious patterns destabilizes the retrieval basin.

For LSR with $b = 3.41$, the load $\alpha = 0.1$ lies below the support threshold $\alpha_{\text{th}} = 0.25$. As predicted, the system remains in the retrieval state across the full temperature range, confirming that no spurious patterns fall within the kernel's support and retrieval is perfect at any temperature. For LSE at the same load, the alignment.
