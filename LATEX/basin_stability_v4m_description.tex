...
\title{Basin Stability Protocol for Phase Diagram Measurement\\
       ...
       {\large v4m: Modified Pattern Generation}}
\author{}
\date{}
...
\subsection{LSR implementation: pattern reduction (v4m)}
\label{sec:mc-lsr}
...
\subsubsection{Critical correction (v4m): Preserving Poisson statistics under $N_{\min}$ enforcement}

The original v4 enforced a minimum dimension $N_{\min} = 50$ by taking $N = \max(N_{\mathrm{v3}}, N_{\min})$, then recomputing $M = \exp(\alpha N)$. This inadvertently changed the Poisson mean to $\lambda = M_{\mathrm{recomputed}} \cdot p_{\text{tail}}$, which differs from the intended $\lambda = M_{\mathrm{v3}} \cdot p_{\text{tail}}$. Since $M$ can exceed $10^{12}$, this shifted $\lambda$ substantially, reducing the sampled $K$ and causing the basin to appear artificially stable (spurious patterns were undersampled).

v4m corrects this by decoupling $N$ and $M$:
\begin{itemize}
    \item Use $N_{\mathrm{v3}} = \lfloor \ln M_{\mathrm{v3}} / \alpha \rfloor$ from the v3 pattern-count schedule (Table~\ref{tab:mc-params}).
    \item Enforce $N = \max(N_{\mathrm{v3}}, N_{\min})$ for better finite-size convergence.
    \item \textbf{Crucially:} keep $M = M_{\mathrm{v3}}$ (do NOT recompute from enforced $N$) for the Poisson mean.
\end{itemize}

This preserves the intended Poisson distribution of active spurious patterns: $\lambda = M_{\mathrm{v3}} \cdot p_{\text{tail}}$, ensuring accurate sampling of basin stability even when $N$ is increased for convergence.
...
v4m uses the same parameters as v4, but corrects the Poisson mean computation to use $M_{\mathrm{v3}}$ (not recomputed from enforced $N$).}
\label{tab:mc-params}
\small
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l l l}
\toprule
Parameter & LSE & LSR (v4m) \\
\midrule
...
$M(\alpha)$         & $20{,}000$--$500{,}000$ \;(power-law, $\gamma\!=\!10$) & $M_{\mathrm{v3}}$ \;(v3 schedule, not recomputed) \\
$N$ range           & $24$--$990$ \;($N = \ln M / \alpha$) & $50$--$990$ \;($N_{\min} = 50$ enforced) \\
Poisson mean        & --- & $\lambda = M_{\mathrm{v3}} \cdot p_{\text{tail}}$ \;(preserved from v3) \\
