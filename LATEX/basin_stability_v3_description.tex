...
\title{Basin Stability Protocol for Phase Diagram Measurement\\
       in Continuous Dense Associative Memory (v3)}
\author{}
\date{}
...
Monte Carlo validation at finite $N$ faces two distinct challenges:
(i)~maintaining the exponential-capacity relationship $P = e^{\alpha N}$ at each grid point, and
(ii)~avoiding metastability artifacts near the first-order retrieval--disorder transition.
Our v3 protocol addresses both through an adaptive-$N$ scheme and a basin stability measurement framework.

\section{Why Basin Stability Instead of Thermal Equilibrium}

Earlier approaches (v1, v2) attempted to measure the thermal equilibrium alignment $\langle\varphi\rangle_{\mathrm{eq}}$ by initializing near the target pattern and running long equilibration.
This failed for two reasons:

\paragraph{Metastability at first-order transitions.}
The retrieval--disorder transition is first order: the free energy has two minima separated by a barrier $\Delta F \sim N^\gamma$.
The Kramers escape time $\tau_{\mathrm{escape}} \sim e^{\Delta F/T}$ grows exponentially with $N$, so larger systems become \emph{harder} to equilibrate.
Finite-size scaling then extrapolates an increasing metastable artifact, producing unphysical $\varphi > 1$ in the critical region.

\paragraph{Low-temperature plateau.}
At low $T$, the acceptance rate drops and the Markov chain freezes before reaching thermal equilibrium.
With fixed equilibration time ($n_{\mathrm{eq}} = 50{,}000$), all runs below $T \approx 0.19$ converge to the same frozen state, creating an artificial plateau.

\paragraph{Basin stability resolves both issues.}
Instead of seeking thermal equilibrium, we ask: \textit{Is the retrieval basin stable against finite perturbations?}
...
The v3 protocol has two cleanly separated phases for each $(\alpha, T)$ point.
...
The key v3 improvement over v2 is this split: by measuring $\varphi$ only during the sampling phase, the observable is not polluted by the initial transient from $\varphi_{\mathrm{init}}$.
...
The simulation processes $\alpha$ values sequentially (or in memory-limited chunks), while all $n_T = 50$ temperatures and $n_{\mathrm{trials}}$ trials are batched on the GPU simultaneously.
...
For LSR with $b = 2 + \sqrt{2}$, the alignment drops from ${\approx}\,1$ at $T \to 0$ to ${\approx}\,0.77$ at $T = 2.5$---even though the retrieval basin remains the unique stable state at all temperatures (for $\alpha < \alpha_{\mathrm{th}}$).
For LSE, the Boltzmann average follows Eq.\,33: $\varphi_{\mathrm{LSE}}(T) = \tfrac{1}{2}[-T + \sqrt{T^2 + 4}]$, which drops more steeply (reaching ${\approx}\,0.62$ at $T = 2.5$).
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{boltzmann_comparison_alpha01.png}
\caption{Boltzmann equilibrium within the LSR retrieval basin at $\alpha = 0.1$.
...
\begin{table}[h]
\centering
\caption{Basin stability Monte Carlo parameters (v3).
...
$T$ grid            & $0.025{:}0.05{:}2.50$, linear \;(50 values) \\
...
\section{Summary of Protocol Evolution}

The v3 basin stability protocol represents the final methodology, refined through several iterations:

\begin{enumerate}
    \item \textbf{v1 (thermal equilibrium, adaptive-$N$)}: Correct phase-space coverage but suffered from metastability above $T_c$ and low-$T$ freezing.
    Finite-size scaling extrapolation amplified metastability artifacts.

    \item \textbf{v2 (thermal equilibrium, unified protocol)}: Standardized LSE and LSR parameters.
    Identified the ``blue bay'' artifact in LSR due to hard-wall metastability.

    \item \textbf{v3 (basin stability, split equilibration/sampling)}: Replaced the thermal-equilibrium question with basin stability.
    Random initial alignment $\varphi \in [0.75, 1.0]$ eliminates the freezing plateau.
    Split equilibration/sampling prevents transient contamination of $\varphi$.
    Massive pattern counts ($P$ up to $500{,}000$) ensure the noise floor and spin-glass transition are correctly realized.
    Unified protocol for both kernels.
\end{enumerate}
