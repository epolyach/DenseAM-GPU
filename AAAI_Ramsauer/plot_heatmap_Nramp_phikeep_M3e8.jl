...
Output: panels_paper/heatmap_Nramp_M3.0e8_phikeep0.40[_bd<th>].{png,pdf}
=#
...
p = heatmap(αs, Ts, Z,
            xlabel = "α",
            ylabel = "T",
            title  = "⟨φ⟩ − φ_eq(T), semismart Nramp (M=3.0e8, φ_keep=0.40)",
            c      = :RdBu,
            clims  = (-0.9, 0.9),
...
if BOUNDARY_TH !== nothing
    ...
    if !isempty(α_bd)
        bd_csv = joinpath(@__DIR__, "panels_paper",
                          @sprintf("heatmap_Nramp_M3.0e8_phikeep0.40_bd%.3g.csv",
                                   BOUNDARY_TH))
        open(bd_csv, "w") do f
...
out_png = joinpath(outdir, "heatmap_Nramp_M3.0e8_phikeep0.40$(suffix).png")
out_pdf = joinpath(outdir, "heatmap_Nramp_M3.0e8_phikeep0.40$(suffix).pdf")
