...
Gaussian approximation as the likely source of error. Brute-force Monte
...
\begin{equation}
    ...
    \label{eq:spinodal}
\end{equation}
At fixed $T$, the saddle-dominated capacity is the load at which the
retrieval free energy
$\fret(T) = (1 - \varphi_{\rm eq}) - \tfrac{T}{2}\ln(1 - \varphi_{\rm eq}^2)$
equals the saddle-valley value, giving
\begin{equation}
    \acSd(\Bnet, T) \;=\; \Bnet\,[\,1 - \fret(T)\,] - g_{\max}(\Bnet),
    \label{eq:capacity_T}
\end{equation}
which reduces to~\eqref{eq:capacity_T0} at $T = 0$.
...
We compare three Metropolis schemes on $\bx \in \sqrt N \cdot S^{N-1}$ at
$\Bnet = 1$.  Each step proposes $\bx + \sigma\,\boldsymbol\eta$ with
$\boldsymbol\eta \sim \mathcal{N}(0, I_N)$ and rescales the result to
$|\bx| = \sqrt N$.  The $(\alpha, T)$ grid in this paper is
$\alpha \in \{0.20, 0.21, \dots, 0.65\}$ (46 loads) and
$T \in \{0.005, 0.015, \dots, 0.495\}$ (50 temperatures).  The schemes
differ in how they represent the $M = \e^{\alpha N}$ patterns inside the
LSE log-sum.
...
every step.  Cost per step is $\mathcal{O}(M N)$, dominated by the GEMM
that produces the $M$ pattern-state overlaps.  We fix the budget at
$M = 4.4 \times 10^6$ and slide $N$ with $\alpha$ along the ramp
$N(\alpha) = \lceil \log M / \alpha \rceil$ (floor $N \geq 12$), so the
Full sweep matches the Cone sweep cell-by-cell.  The ramp runs from
$N = 76$ at $\alpha = 0.20$ to $N = 24$ at $\alpha = 0.65$.  At each
...
steps.  Per cell we record the time-averaged target overlaps
$\varphi_a, \varphi_b$, the replica overlap $q_{ab}$, and the maximum
non-target overlap.  The full sweep takes $\approx 26$ days on a single
NVIDIA RTX A6000 at \texttt{Float32}.

\paragraph{Cone.}
A fixed alignment cutoff $\phi_{\mathrm{keep}}$ partitions the pattern
set into two pieces.  Patterns with $\varphi_{1\mu} \geq \phi_{\mathrm{keep}}$
...
the spherical density~\eqref{eq:hphi}.  In the rescaled variable
$y = (1 + \phi)/2$, the spherical density becomes a symmetric Beta and
each in-cone pattern's overlap with $\bxi^1$ is drawn from the truncated
distribution
\begin{equation}
y \sim \mathrm{Beta}\!\Bigl(\tfrac{N-1}{2}, \tfrac{N-1}{2}\Bigr)
   \;\;\text{truncated to}\;\; y \in \bigl[(1 + \phi_{\mathrm{keep}})/2,\,1\bigr],
\qquad \varphi_{1\mu} = 2y - 1,
\label{eq:beta-tail}
\end{equation}
together with a uniform perpendicular direction on the $(N{-}2)$-sphere
orthogonal to $\bxi^1$.  The cone set is fixed once per disorder.  We
use $M = 3 \times 10^8$ and $\phi_{\mathrm{keep}} = 0.40$ with the same
$N(\alpha)$ ramp formula as Full; the ramp runs from $N = 98$ at
$\alpha = 0.20$ to $N = 30$ at $\alpha = 0.65$, and the typical in-cone
count grows from $K \sim 7 \times 10^3$ at $\alpha = 0.20$ to
$K \sim 4 \times 10^6$ at $\alpha = 0.65$, i.e. about 2\%--5\% of $M$
typically and never more than 5\% at any cell.  Restricting the explicit
pattern set to this small cone is what makes the scheme tractable at
$M = 3 \times 10^8$; the remaining $\sim 98\%$ of the patterns enter
through the closed-form bulk integral.  Disorder count, replicas, initial
condition, step size, equilibration and sampling lengths, and recorded
observables are identical to Full.  At the shared budget
$M = 4.4 \times 10^6$, $N = 76$, Full and Cone boundaries agree across
the $\alpha$ range we measure, so the bulk patterns are genuinely outside
the basin physics rather than silently dropped.  The full Cone sweep at
$M = 3 \times 10^8$ takes $\approx 7$ days on the same A6000.

\paragraph{Bulk.}
No patterns are generated.  Cone, taken in the limit $\phi_{\mathrm{keep}}
\to 1$, admits no patterns inside the cone at all; the integral in the
bulk constant~\eqref{eq:cbulk} simultaneously covers the entire spherical
density, and in the large-$N$ limit it is dominated by its saddle point,
giving the closed form $M\,\e^{N\gmax(\Bnet)}$.  We name this scheme
\emph{Bulk} because the log-sum it leaves the chain with consists of
exactly two terms, the target and the saddle-evaluated bulk constant: no
pattern survives explicitly.  The chain energy depends on $\bx$ only
through the target overlap $\varphi_1 = \bxi^1 \cdot \bx / N$:
\begin{equation}
\ln Z(\varphi_1) = \ln\!\bigl[ \e^{\Bnet N \varphi_1}
                + \e^{N (\alpha + \gmax(\Bnet))} \bigr].
\label{eq:bulk-logZ}
\end{equation}
Cost per step is $\mathcal{O}(N)$, set by the proposal and the sphere
projection, with no $M$-dependence at all.  The chain starts inside the
basin at $\varphi_1 = \phieq(T)$ with perpendicular components uniform on
the $(N{-}1)$-sphere of radius $\sqrt N \cdot \sqrt{1 - \phieq^2}$; an
init at $\varphi_1 = 1$ stalls because $\sigma$ sized for basin diffusion
is too aggressive for the linear slope at the sphere edge.  The step
size $\sigma = \sqrt{2 T / \gamma_{\rm eq}}$ with
$\gamma_{\rm eq} = T(1 + \phieq^2)/(1 - \phieq^2)^2$ is set from the
basin curvature so that the per-step energy change is $\mathcal{O}(T)$
at any $N$, giving $N$-independent acceptance.  There is no disorder;
trial randomness comes only from the MC RNG.  Per $\alpha$ we scan $T$
downward from $\min(T_{\rm sp}(\alpha), 0.495)$ in steps of $0.01$,
running 64 independent chains for $T_{\mathrm{MC}} = 10^5$ steps each,
and record the boundary $T_{\rm bd}(\alpha)$ as the largest $T$ at which
the residual $\phieq(T) - \avg{\varphi_1}$ first stays below $0.02$ for
three consecutive $T$-steps.  We run Bulk at $N \in \{100, 300, 10000\}$
to expose the kinetic boundary as a function of dimension; on a single
CPU node with 16 threads the three sweeps take about a minute, about
five minutes, and about ninety minutes respectively.

\paragraph{$N$-reach and what each scheme tests.}
Cone at fixed $\phi_{\mathrm{keep}}$ keeps
$K = M\,\Pr(\phi \ge \phi_{\mathrm{keep}}) \sim
\e^{N[\alpha - I(\phi_{\mathrm{keep}})]}$ patterns inside the cone, with
$I(\phi) \equiv -\tfrac12 \ln(1 - \phi^2)$ the large-$N$ rate of the
spherical tail.  At $\phi_{\mathrm{keep}} = 0.4$, $I \approx 0.087$;
for any $\alpha > 0.087$ the in-cone count grows exponentially in $N$ and
the GPU budget is exhausted around $N \sim 30$--$50$ at the
$M = 3 \times 10^8$ Cone budget.  Bulk sets the cone aperture to zero
($\phi_{\mathrm{keep}} \to 1$) and the bulk integral to its saddle value
$M\,\e^{N\gmax(\Bnet)}$, so $K = 0$ by construction; the chain energy
reduces to the leading-order log-sum~\eqref{eq:bulk-logZ} and runs at
$\mathcal{O}(N)$ cost per step, decoupling from $M$ entirely.

The three schemes probe different physics.  Bulk realises the
Kramers kinetics of~\eqref{eq:F-LO} at large $N$, but the analytic
bulk constant $M\,\e^{N\gmax(\Bnet)}$ is taken on faith and any correction
to $\gmax$ would be invisible.  Cone with $\phi_{\mathrm{keep}}$ near or
below the integrand saddle $\phi^{*}(\Bnet)$ retains the patterns that
dominate the spurious-sum integral and audits the bulk directly; a
subleading correction to $\gmax$ would appear as a mismatch between Cone
and Full boundaries.  At the shared budget $M = 4.4 \times 10^6$ the two
...
A faster scheme (sometimes called ``smart'') refreshes the in-cone set
every $N_{\mathrm{refresh}}$ steps from a cone around the current chain
position, keeping only the patterns whose alignment with $\bx$ is
...
reasons unrelated to~\eqref{eq:acsd}.  Full, Cone, and Bulk all avoid
the issue: Full uses every pattern at every step, Cone fixes the in-cone
set once per disorder, and Bulk carries no patterns at all.
...
Fig.~\ref{fig:heatmap} shows the empirical $(\alpha, T)$ stability diagram at
...
basin, red where it escapes to the saddle valley. The blue staircases trace,
for each $\alpha$ column, the lowest $T$ at which roughly 5\% of chain
endpoints have escaped from the retrieval basin to the saddle valley.
Operationally, this is the threshold
$\varphi_{\mathrm{eq}}(T) - \langle\varphi\rangle > 0.02$ at the empirical
basin-to-saddle gap $\Delta\varphi \approx 0.40$ along the boundary, after
a light smoothing pass to suppress single-cell MC noise. At fixed $\alpha$, increasing the budget from $M = 4.4 \times 10^6$ to
$M = 3.0 \times 10^8$ (which raises the reachable $N$ via the $N$-ramp)
shifts the empirical boundary to higher $T$. The expected destination at
...
is still far from the spinodal.  It happens to lie close to the saddle
line $\acSd(\Bnet, T)$~\eqref{eq:capacity_T}, but this proximity is
incidental: the saddle line is the thermodynamic crossover where the basin
and the saddle valley have equal free energy, and chains in an MC ensemble
redistribute continuously between the two minima as $\alpha$ crosses it,
with no sharp change in the retrieval fraction.  The Gaussian prediction
...
and the chain follows the two-term log-sum~\eqref{eq:bulk-logZ} directly,
...
\caption{Empirical $(\alpha, T)$ stability diagram at $\Bnet = 1$. Heatmap:
...
the spinodal is drawn last to make this overlap visible.  Reference curves:
