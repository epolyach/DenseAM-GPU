#=
GPU-Accelerated SEMISMART LSE Basin Stability — AAAI 2027  ·  N(α)-RAMP
────────────────────────────────────────────────────────────────────────────────
Goal: test whether the data boundary converges to the analytical red curve
α_c^sd(T) when each α is run at the largest N that keeps the pattern set
near a fixed budget M_TARGET. Anchored so α=0.70 gives N=25 (matching the
existing honest_AAAI baseline).

Scheme
  • M_TARGET = 4.4·10⁷    (≈ exp(0.70·25))
  • N(α)     = round(log(M_TARGET) / α)
  • Semismart truncation: per (N, α), binary-search PHI_KEEP so that
    M·P(φ ≥ PHI_KEEP) ≈ K_TARGET. Direct conditional sampling on the
    truncated Beta((N-1)/2,(N-1)/2) density (no rejection-over-M).
  • Disorder chunking: target N_DIS_TARGET samples per α. Per-α chunk
    size n_dis_chunk derived from MEM_BUDGET_GB. Loop chunks until target
    met; each chunk advances the disorder index in the CSV.

Same energy / MC step as basin_stab_LSE_semismart_AAAI.jl. CSV schema
matches it so plot_LSE_AAAI_heatmap.jl reads this file unchanged.

Usage
  julia basin_stab_LSE_semismart_AAAI_Nramp.jl            # resume
  julia basin_stab_LSE_semismart_AAAI_Nramp.jl --fresh    # overwrite

Output: basin_stab_LSE_semismart_AAAI_Nramp.csv
────────────────────────────────────────────────────────────────────────────────
=#
...
# ──────────────── N(α) ramp + truncation budget ────────────────
const M_TARGET     = 4.4e6
const N_FLOOR      = 12
const K_TARGET     = 30_000
const K_HARD_CAP   = 200_000
const PHI_KEEP_MIN = 0.10
const PHI_KEEP_MAX = 0.97
...
# :honest  → no truncation (M ≤ K_TARGET); patterns are full pattern set.
# :smart   → PHI_KEEP picked so M·P(φ≥PHI_KEEP) ≈ K_TARGET.
# :infeasible → even at PHI_KEEP_MAX the retained count exceeds K_TARGET.
...
const csv_out     = "basin_stab_LSE_semismart_AAAI_Nramp_M4.4e6.csv"
...
function read_disorder_progress(csv_file::String)
    ...
    counts = Dict{String,Int}()      # α-string → max disorder index seen
    first_data = true
...
function main()
    ...
    println("="^76)
    println("SEMISMART LSE Basin Stability — AAAI 2027  ·  N(α)-RAMP")
    @printf("  M_TARGET = %.1e   N(α) = round(log(M)/α)   anchor: α=0.70 → N=25\n", M_TARGET)
    @printf("  K_TARGET = %d   N_DIS_TARGET = %d   MEM_BUDGET = %.1f GB\n",
            K_TARGET, N_DIS_TARGET, MEM_BUDGET_GB)
    if n_alpha > 1
    ...
    for i in 1:n_alpha
        ...
        plan[i] = (alpha_idx=i, α=α, N=N, phi_keep=phi_keep, K_alloc=K_alloc,
                   status=status, M_full=M_full, p_above=p_above, chunk=chunk)
        @printf("  α=%.2f  N=%-3d  M=%-10d  status=%-11s  φ_keep=%+5.3f  K_alloc=%-7d  chunk=%d\n",
                α, N, M_full, String(status), phi_keep, K_alloc, chunk)
    end
    println()

    if FRESH_START || !isfile(csv_out)
        open(csv_out, "w") do f
            @printf(f, "# generator=basin_stab_LSE_semismart_AAAI_Nramp.jl  M_TARGET=%.1e  K_TARGET=%d  N_FLOOR=%d  betanet=%s\n",
                    M_TARGET, K_TARGET, N_FLOOR, betanet)
    ...
    for p in plan
        p.status == :infeasible && (println("Skip α=$(p.α): infeasible"); continue)
        αkey = @sprintf("%.3f", p.α)
