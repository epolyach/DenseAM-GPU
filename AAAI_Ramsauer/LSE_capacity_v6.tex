...
for several $T$. RB: retrieval basin; SB: spurious basin (where the spurious term
...
ninety minutes per sweep.  We have verified Cone-Full equivalence at
...
At low $T$ the Cone boundary in Fig.~\ref{fig:heatmap} sits to the
right of the zero-temperature capacity
$\ac(\Bnet, 0) = \Bnet - \gmax(\Bnet) = 0.6226$ at $\Bnet = 1$,
reaching $\alpha \approx 0.70$.  This is a finite-$N$ effect.  The
softmax sum~\eqref{eq:softmax_sum}, with the spurious term replaced
by its Laplace value~\eqref{eq:spurious_laplace} from Appendix~A, defines the
finite-$N$ effective potential at fixed disorder,
\begin{multline}
...
\label{eq:V-finite-N}
\end{multline}
where the entropy coefficient $(N-3)/(2N)$ comes from the
exact spherical marginal $h(\phi) \propto (1-\phi^2)^{(N-3)/2}$
in~\eqref{eq:exact_density}, not its $N \to \infty$ limit
$1/2$.  The $N \to \infty$ limit of~\eqref{eq:V-finite-N} is the
LO landscape
\begin{equation}
    \frac{V_N(\phi)}{N} \to 1 -\max(\phi, \varphi_c) - \tfrac{T}{2}\ln(1-\phi^{2}),
\end{equation}
and at finite $N$ the log-sum smooths the LO corner over width
$(\Bnet N)^{-1}$.
Whenever the cusp $\varphi_c$ sits just above $1$ (the regime
$\alpha > \ac(\Bnet, 0)$), the smoothing reaches onto the sphere
edge and produces a local minimum of $V_N$ at $\phi$ just below
$1$ that the LO curve does not have.  Fig.~\ref{fig:Veff_finite_N}
shows this at the three loads $\alpha = 0.62, 0.65, 0.70$ for the
$N(\alpha)$ at $M = 4.4 \times 10^6$.

The local-minimum position of $V_N$ matches the steady-state
alignment of cold-initialised chains from the Full/Cone run in the
same $(\alpha, T)$ cells to within $0.002$.  For $\alpha = 0.62, 0.65$ it is the global minimum
of $V_N$; for $\alpha = 0.70$ it is metastable with a small
per-chain Boltzmann exponent ($\approx 1.3$ at $T = 0.005$) and no
chain in the Full/Cone ensembles escapes within $T_{\rm MC}$.  Setting
$V_N' = V_N'' = 0$ gives a finite-$N$ correction to the
zero-temperature capacity (Appendix~C),
\begin{equation}
\ac^{\mathrm{fN}}(\Bnet, T, N) \;=\; \ac(\Bnet, 0)
   \;-\; \frac{1 + \ln\!\big(T\,\Bnet\,(N-3) / [2\sqrt{1-\varphi^{*4}}]\big)}{N},
\label{eq:capacity_finiteN}
\end{equation}
positive for $T\,\Bnet\,(N-3) < 2\sqrt{1-\varphi^{*4}}/\e$ and
vanishing as $1/N$ at fixed $T$.  At $\Bnet = 1, T = 0.005$ this
gives $\ac^{\mathrm{fN}} = 0.712$ at $N(0.70) = 22$ (Cone
$M = 4.4 \times 10^6$) and $0.683$ at $N(0.70) = 28$ (Cone
$M = 3 \times 10^8$): the right edge of the heatmap retracts from
$\alpha \approx 0.71$ at the small budget to $\alpha \approx 0.68$
at the large budget, matching the empirical $5\%$ blue staircases in
Fig.~\ref{fig:heatmap}.
...
\section{Appendix C: Finite-$N$ correction to the zero-temperature capacity}
\label{sec:appendix_C}

We derive the finite-$N$ correction~\eqref{eq:capacity_finiteN}
quoted in the Results.

Differentiating~\eqref{eq:V-finite-N} in $\phi$ gives
\begin{equation}
\frac{1}{N}\,\frac{dV_N}{d\phi}
   \;=\; -\varsigma\!\Big(\Bnet N(\phi - \varphi_c)
                       + \tfrac{1}{2}\ln(1-\varphi^{*4})\Big)
   \;+\; \frac{T\,(N-3)\,\phi}{N(1-\phi^{2})},
\label{eq:V-finite-N-deriv}
\end{equation}
with $\varsigma(x) = 1/(1 + \e^{-x})$ the logistic and
$\varphi_c = (\alpha + \gmax(\Bnet))/\Bnet$ from~\eqref{eq:cusp}.
The $\tfrac{1}{2}\ln(1-\varphi^{*4})$ shift inside the logistic is
the $\ln$ of the Appendix~A prefactor.  The factor $(N-3)/N$ on the
entropy slope comes from the exact spherical
marginal~\eqref{eq:exact_density}, kept unexpanded.  A critical point
of $V_N$ balances the smoothed jump against the sphere-entropy slope.

For $\varphi_c > 1$ (equivalently $\alpha > \ac(\Bnet, 0)$) the LO
floor $-\varphi_c$ sits off the unit sphere; the smoothing still
reaches a neighbourhood of $\phi = 1$ where the entropy slope
diverges, so the balance holds in a window of width
$\sim 1/(\Bnet N)$ below $\phi = 1$ and admits two solutions (the
local minimum and the adjacent barrier saddle) that merge as $N$
decreases.  At the merger, $V_N' = V_N'' = 0$ simultaneously; in
the regime $T\,\Bnet\,(N-3) \ll 1$ this two-equation system reduces
to
\begin{equation}
\Bnet N\,(\varphi_c - 1) \;+\;
   \ln\!\Bigg(\frac{T\,\Bnet\,(N-3)}{2\sqrt{1-\varphi^{*4}}}\Bigg) \;+\; 1
\;=\; 0.
\label{eq:bifurcation}
\end{equation}
Solving~\eqref{eq:bifurcation} for $\varphi_c$ and substituting
$\varphi_c = (\alpha + \gmax)/\Bnet$ together with
$\ac(\Bnet, 0) = \Bnet - \gmax(\Bnet)$~\eqref{eq:capacity_T0}
gives~\eqref{eq:capacity_finiteN}.  For $\varphi_c \le 1$ no
bifurcation occurs: the LO basin already sits on the unit sphere
and the residual minimum exists for all $N$.

Numerical values at $\Bnet = 1, T = 0.005$ ($\varphi^* \approx 0.618$,
prefactor $1/\sqrt{1-\varphi^{*4}} \approx 1.082$):
$\ac^{\mathrm{fN}}(22) = 0.712$, $\ac^{\mathrm{fN}}(24) = 0.700$,
$\ac^{\mathrm{fN}}(28) = 0.683$, $\ac^{\mathrm{fN}}(30) = 0.676$,
$\ac^{\mathrm{fN}}(100) = 0.626$.  The Cone run at
$M = 4.4 \times 10^6$ reaches $N(0.65) = 24$ and $N(0.70) = 22$;
both are below the local thresholds, so the residual basin is
visible at both loads in Fig.~\ref{fig:Veff_finite_N}.  The Cone
run at $M = 3 \times 10^8$ reaches $N(0.65) = 30$ and
$N(0.70) = 28$: at $\alpha = 0.65$
the basin survives ($\ac^{\mathrm{fN}}(30) = 0.676 > 0.65$), at
$\alpha = 0.70$ it disappears ($\ac^{\mathrm{fN}}(28) = 0.683 < 0.70$).
