...
(hereafter, $\varphi$) as a Gaussian with mean $0$ and variance $1/N$, and asks at
...
near $\varphi = 0$, but the densities diverge near $\varphi = 1$.
...
\label{eq:hphi}
\end{equation}
At large $N$, $h$ concentrates near $\varphi = 0$ with variance $1/N$,
...
\qquad
\frac{1}{N}\ln S
\;\approx\; \alpha + \frac{1}{N}\ln\!\int_{-1}^{1} h(\varphi)\,
...
$M\int_{\varphi_{\max}}^{1} h(\varphi)\,\dd\varphi = 1$, which after computing the right
...
method,
\begin{equation}
\int_{-1}^{1} h(\varphi)\,\e^{N\Bnet\varphi}\,\dd\varphi
...
\;\to\; \max\!\bigl(\phi_1,\;\varphi_c(\alpha, \Bnet)\bigr),
\qquad
...
\;=\; -\max\!\bigl(\phi_1, \varphi_c\bigr)
   \;-\; \tfrac{T}{2}\ln(1 - \phi_1^2).
\label{eq:F-LO}
\end{equation}
The two branches meet in a cusp at $\phi_1 = \varphi_c$.  We call the
...
slope) the \emph{saddle valley}: for $\phi_1 < \varphi_c$ the bulk
...
alignment with $\bxi^1$ exceeds a cutoff $\varphi_{\mathrm{keep}}$.  The
...
h(\varphi)\,\e^{N\varphi}\,\dd\varphi$.  The retained set is fixed once
...
the spherical density~\eqref{eq:hphi} restricted to $\varphi > \varphi_c$.
