...
\subsection{The small-$\alpha$ discrepancy: a Kramers candidate}
...
We do not validate this hypothesis numerically in this paper. A minimal
validation programme is fivefold. (i)~\emph{Chain-length scan}: at
...
hopping. Any subset of (i)--(v) would either extend this paper or
...
empirical Monte Carlo transition and the saddle prediction is
plausibly, but only plausibly, the LSE analogue of the LSR Kramers
shift of~\citep{petrova2026metastable}; the explicit validation
programme of \Cref{sec:mc:smallalpha} would either confirm this
interpretation as a finite-$N$ kinetic artefact or expose a genuine
thermodynamic finite-$N$ correction to the saddle calculation that
