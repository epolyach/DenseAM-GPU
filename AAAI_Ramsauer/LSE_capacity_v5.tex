...
The hardware allows maximum budget at
$M = 4.4 \times 10^6$ which was kept fixed, and $N$ is obtained as
$N(\alpha) = \lceil \log M / \alpha \rceil$, from
...
The full sweep takes $\approx 26$ days on a single NVIDIA RTX A6000 at \texttt{Float32}.
...
use $M = 3 \times 10^8$ and $\phi_{\mathrm{keep}} = 0.40$ resulting in $N = 98$ at
$\alpha = 0.20$ to $N = 30$ at $\alpha = 0.65$, and the typical in-cone
count grows from $K \sim 7 \times 10^3$ at $\alpha = 0.20$ to
$K \sim 4 \times 10^6$ at $\alpha = 0.65$, i.e. $K/M$ ranging from
$\sim 2\times 10^{-5}$ at $\alpha = 0.20$ to $\sim 1.3\times 10^{-2}$ at
$\alpha = 0.65$, never above $2\%$ at any cell.  Restricting the explicit
pattern set to this small cone is what makes the scheme tractable at
$M = 3 \times 10^8$; the remaining $> 98\%$ of the patterns enter
...
for three consecutive $T$-steps.  There is no disorder.  The three
sweeps take about one, five, and ninety minutes on a single
16-thread CPU node.
...
The samplers differ in their $N$-reach.  Full holds all $M$
patterns explicitly, so the GPU memory cap on $M$ enforces
$N(\alpha) = \lceil \log M / \alpha \rceil$ (an "$N$-ramp" along
$\alpha$) and limits $N \le 25$ at our budget.  Cone retains only
the in-cone subset, $K \sim \e^{N(\alpha - I(\phi_{\rm keep}))}$ with
$I(\phi) = -\tfrac12 \ln(1 - \phi^2)$ the large-$N$ rate of the
spherical tail, extending the reach to $N \le 98$ at
$M = 3 \times 10^8$; $K$ still grows exponentially in $N$, so the cap
remains.  Bulk has $K = 0$ and decouples from $M$ entirely, reaching
$N = 10^4$.  Cone and Full boundaries agree across $\alpha$ at the
shared budget $M = 4.4 \times 10^6$, so the analytic bulk constant
in~\eqref{eq:bulk-logS} captures the spurious physics to within MC
