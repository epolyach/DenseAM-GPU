...
\label{eq:lse}
\end{equation}
with $\Bnet$ the kernel sharpness (inverse variance, unrelated to the
external temperature $T$), and stores $M = \e^{\alpha N}$ patterns at
load $\alpha = \ln M / N$.  The question is: what is the largest
$\alpha$ at which the chain initialised at $\bxi^1$ stays close to
$\bxi^1$?
...
kernel sharpness, with saddle value $\gmax(\Bnet)$.  At $\Bnet = 1$ —
the value used in the Monte Carlo of \Cref{sec:mc,sec:results} — the
saddle is the golden ratio $\varphi^{*} = (\sqrt 5 - 1)/2$,
...
= -\varphi_1$ with $\varphi_1 = \bxi^1 \cdot \bx / N$ — the $\Bnet$
factor cancels for a single-pattern dominant term, so $H/N$ at retrieval
is $\Bnet$-independent.  Define the energy per spin relative to the
retrieval baseline, $u(\varphi_1) = 1 - \varphi_1$, and the
$(N\!-\!1)$-sphere entropy per spin $s(\varphi_1) = \tfrac12 \ln(1 -
\varphi_1^2)$.  The retrieval free energy is the minimum of
$f(\varphi_1, T) = u - T s$ (also $\Bnet$-independent for the same
reason):
\begin{equation}
...
\label{eq:Sint}
\end{equation}
We evaluate~\eqref{eq:Sint} by the saddle-point method in the large-$N$
limit (Laplace's method): the integrand concentrates at the maximiser
$\varphi^{*}(\Bnet)$ of the exponent
$g(\varphi; \Bnet) = \tfrac12 \ln(1 - \varphi^2) + \Bnet\,\varphi$, and
\begin{equation}
\int \e^{N g(\varphi; \Bnet)} \dd\varphi
   \;\sim\; \sqrt{\frac{2\pi}{N\,|g''(\varphi^{*})|}}\,
            \e^{N\,g(\varphi^{*}; \Bnet)}
\;\Longrightarrow\;
\frac{1}{N} \ln S \;\to\; \alpha + \gmax(\Bnet)
\;\;(N \to \infty),
\label{eq:saddle-method}
\end{equation}
with subleading $\log N / N$ corrections.  The saddle condition
$g'(\varphi^{*}) = 0$ gives $\Bnet(1 - \varphi^{*\,2}) = \varphi^{*}$,
with positive root
\begin{equation}
