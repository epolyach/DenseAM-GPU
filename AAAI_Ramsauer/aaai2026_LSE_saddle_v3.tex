...
\paragraph{Hybrid.}
Same kept-tail + analytic-bulk decomposition as truncated, but the
chain is reduced to its scalar overlap $\varphi_1$ and the bulk constant
is set by the saddle of~\eqref{eq:Sint}.  Concretely: patterns with
overlap $\varphi_{1\mu} > \varphi_c = [\alpha + \gmax(\Bnet)]/\Bnet$ are sampled
from the spherical Beta tail and kept fixed per disorder (count
$K_{\mathrm{cusp}} \sim M \cdot \Pr(\varphi > \varphi_c)$; in our $N = 100$,
$\Bnet = 1$ runs, between $\sim 4500$ at $\alpha = 0.50$ and $\sim 8500$
at $\alpha = 0.62$).  The contribution of the bulk ($\varphi_{1\mu} < \varphi_c$)
is replaced by the $\varphi_1$-independent constant
$M\,\e^{N \gmax(\Bnet)}$ from~\eqref{eq:gmax}.  Metropolis runs in 1D on
$\varphi_1$ with the $(N\!-\!1)$-sphere entropy $\tfrac12 \ln(1 -
\varphi_1^2)$ as the prior.  The physics is the same as truncated; the
gain is one scalar update per MC step instead of an $N$-dimensional spin
update, which makes the scheme cheap enough to push to $N = 100$ near
$\alpha_c$ where truncated MC at the full spin becomes slow.
