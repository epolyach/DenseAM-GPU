...
# ──────────────── Read data ────────────────
# Use extrapolated CSVs if available, otherwise fall back to base
lse_csv = isfile("lse_extrap.csv") ? "lse_extrap.csv" : "lse_alpha_sweep_table.csv"
lsr_csv = isfile("lsr_extrap.csv") ? "lsr_extrap.csv" : "lsr_alpha_sweep_table.csv"
println("LSE data: $lse_csv")
println("LSR data: $lsr_csv")

data_lse = CSV.read(lse_csv, DataFrame)
data_lsr = CSV.read(lsr_csv, DataFrame)
...
# Curved portion: only where α_c > α_th
...
# Vertical line at α_th from T_max UP to max(T)
if !isnan(T_max)
    plot!(p2, [alpha_th, alpha_th], [T_max, maximum(T_values)],
          color=:black, linewidth=2.5, label=false)
end

# Dashed vertical at α_c(0) = 0.5
vline!(p2, [0.5],
...
savefig(fig, "maps.png")
println("Combined plot saved: maps.png")
println("α_th = $(round(alpha_th, digits=4)),  T_max = $(round(T_max, digits=4))")
