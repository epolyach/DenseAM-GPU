#=
GPU-Accelerated LSR Alpha Sweep Table Generation
...
- Output: lsr_alpha_sweep_table.csv (same format as CPU version)
=#
...
function compute_energy_lsr!(E::CuVector{F}, x::CuArray{F,3},
    ...
    @. s = max(s, F(1e-10))                         # avoid log(0)
    E .= vec(@. -Nb * log(s))                       # [n_chains]
    return nothing
end
...
function main()
    ...
    println("=" ^ 70)
    println("LSR Alpha Sweep – GPU (Adaptive N, Float32, b=$(round(b_lsr, digits=3)))")
    println("=" ^ 70)
    dev = CUDA.device()
