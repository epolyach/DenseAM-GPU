#=
GPU-Accelerated LSE Alpha Sweep Table Generation
- Adaptive N: N(α) = ln(P(α)) / α, P grows linearly from 200 to 5000
- 256 independent pattern realizations for quenched disorder averaging
- CUBLAS batched gemm for parallel trial computation
- Float32 precision, designed for NVIDIA A6000 (48 GB)
- Output: lse_alpha_sweep_table.csv (same format as CPU version)
=#
...
function compute_energy_lse!(E::CuVector{F}, x::CuArray{F,3},
    ...
    # overlap shape: [P × n_T × N_TRIALS]
    CUDA.CUBLAS.gemm_strided_batched!('T', 'N', one(F), patterns, x, zero(F), overlap)

    # Log-sum-exp trick (in-place on overlap to avoid temporaries)
    @. overlap = -betanet * (Nf - overlap)         # log-arguments
    m = maximum(overlap, dims=1)                    # [1 × n_T × N_TRIALS]
    @. overlap = exp(overlap - m)                   # shifted exponentials
    s = sum(overlap, dims=1)                        # [1 × n_T × N_TRIALS]
    E .= vec(@. -(m + log(s)) / betanet)            # [n_chains]
    return nothing
end
...
function main()
    ...
    println("=" ^ 70)
    println("LSE Alpha Sweep – GPU (Adaptive N, Float32)")
    println("=" ^ 70)
    dev = CUDA.device()
