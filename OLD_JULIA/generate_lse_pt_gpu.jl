#=
GPU-Accelerated LSE Alpha Sweep with Parallel Tempering (Replica Exchange)
...
function replica_swap!(x::CuArray{F,3}, E::CuVector{F},
                       ...
                       np::Int)
    # Reshape E to [n_T × N_TRIALS]
    E_mat = reshape(E, n_T, N_TRIALS)

    # Energy differences for all pairs across all trials
    ΔE = E_mat[j_lo, :] .- E_mat[j_hi, :]            # [np × N_TRIALS]

    # Acceptance probability: exp(Δβ * ΔE)
    log_p = Δβ_gpu .* ΔE                              # [np × N_TRIALS]
    r = CUDA.rand(F, np, N_TRIALS)
    acc = r .< exp.(log_p)                             # [np × N_TRIALS]
...
function main()
    ...
    println("=" ^ 70)
    println("LSE Alpha Sweep – GPU (Parallel Tempering, N_EQ=$N_EQ)")
    println("=" ^ 70)
    dev = CUDA.device()
    @printf("GPU: %s (%.1f GB)\n\n", CUDA.name(dev), CUDA.totalmem(dev)/1e9)

    # Upload Δβ vectors to GPU
    ...
    t0 = time()
    n_swaps_acc = zeros(Int, 2)  # [even_accepted, odd_accepted]
    n_swaps_tot = zeros(Int, 2)  # [even_total, odd_total]
    ...
    for step in 1:N_EQ
        for i in 1:n_alpha
            ...
            # Replica exchange: alternate even/odd
            if iseven(step)
