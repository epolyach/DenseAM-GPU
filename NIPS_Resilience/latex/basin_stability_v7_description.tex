...
Using the normalized overlap $\tilde\varphi = \varphi / \varphi_{\mathrm{eq}}(T)$ and the normalized Edwards--Anderson parameter $\tilde q = q_{\mathrm{EA}} / \varphi_{\mathrm{eq}}(T)^2$, where $\varphi_{\mathrm{eq}}(T)$ is the theoretical equilibrium overlap at temperature~$T$, the $(\alpha, T)$ plane is classified into four phases:

\begin{table}[h]
\centering
\caption{Phase classification thresholds and definitions. All thresholds are applied to normalized quantities.}
\label{tab:phases}
\begin{tabular}{llll}
\toprule
Phase & Symbol & Condition & Physical meaning \\
\midrule
Retrieval      & R  & $\tilde\varphi > \varphi_R$ and $\tilde q > q_{\mathrm{th}}$ & Pattern retrieved, frozen \\
Mixed          & M  & $\varphi_M < \tilde\varphi \le \varphi_R$ and $\tilde q > q_{\mathrm{th}}$ & Partial overlap, frozen \\
Spin-glass     & SG & $\tilde\varphi \le \varphi_M$ and $\tilde q > q_{\mathrm{th}}$ & No retrieval, frozen \\
Paramagnetic   & P  & $\tilde q \le q_{\mathrm{th}}$ & Ergodic, no order \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{align}
    q_{\mathrm{th}} &= 0.15  && \text{(frozen/ergodic boundary on } \tilde q\text{)}\,,\\
    \varphi_R &= 0.99  && \text{(retrieval/mixed boundary on } \tilde\varphi\text{)}\,,\\
    \varphi_M &= 0.1   && \text{(mixed/spin-glass boundary on } \tilde\varphi\text{)}\,.
\end{align}

\noindent
Applying all thresholds to normalized quantities removes the trivial temperature dependence of the equilibrium state: a raw $q_{\mathrm{EA}}$ threshold would misclassify high-temperature retrieval states as paramagnetic, since $q_{\mathrm{EA}} \approx \varphi_{\mathrm{eq}}(T)^2$ decreases with~$T$ even deep in the retrieval phase.
...
\begin{itemize}
    ...
    \item $\tilde q = q_{\mathrm{th}} = 0.15$ (cyan dashed) --- frozen/ergodic boundary.
\end{itemize}
