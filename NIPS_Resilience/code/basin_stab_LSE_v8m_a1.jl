#=
GPU-Accelerated LSE Basin Stability Test (v8m_a1) — PER-SAMPLE OUTPUT
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSE_v8m_a1.jl           # resume from last completed α
  julia basin_stab_LSE_v8m_a1.jl --fresh   # overwrite CSV, start from scratch

Goal (AAAI paper): Test whether LSE retrieval persists for α > 0.5,
  contradicting the Gaussian-approximation prediction α_c(0)=1/2 of
  Ramsauer et al. and confirming the exact-density prediction
  α_c(0)=∞ derived in latex/LSE_gaussian.tex.

  Per disorder sample, saves: φ_a, φ_b, q_12, φ_max_other
  Single CSV output: basin_stab_LSE_v8m_a1.csv

Differences from v8m:
- α grid extended to [0.01, 1.00] with step 0.01 (100 values)
- MAX_PAT raised from 500_000 to 1_200_000  →  N=14 at α=1.0
- M(α) interpolated geometrically (^(1/ind)) as in v8m: 20k → 1.2M
- Output filename: basin_stab_LSE_v8m_a1.csv

Resulting (α, M, N) ramp:
  α=0.01 → M≈20k,   N≈990
  α=0.50 → M≈170k,  N≈24
  α=1.00 → M=1.2M,  N=14
...
const alpha_vec = collect(F(0.01):F(0.01):F(1.0))     # 100 values
const T_vec     = collect(F(0.025):F(0.05):F(2.0))    # 40 values
const n_alpha   = length(alpha_vec)
const n_T       = length(T_vec)
const MIN_PAT   = 20000
const MAX_PAT   = 1_200_000           # log(1.2e6) ≈ 13.998 → N=14 at α=1.0
const ind       = 10
...
function main()
    ...
    println("=" ^ 70)
    println("LSE Basin Stability Test – GPU v8m_a1 (β_net=$(betanet), per-sample output)")
    println("  v8m_a1: α extended to [0.01, 1.00], MAX_PAT raised to 1.2M (N=14 at α=1)")
    println("  Per-disorder-sample φ_a, φ_b, q_EA, φ_max_other")
    ...
    println("  M range: $MIN_PAT – $MAX_PAT (interpolated as M^(1/$ind))")
    println("=" ^ 70)
    dev = CUDA.device()
    ...
    csv_out = "basin_stab_LSE_v8m_a1.csv"
