#=
Residual heatmap for v20 CPU prototype.
Reads basin_stab_LSE_smart_v20_cpu_N<N>.csv and plots
    Δ(α, T) = ⟨φ⟩(α, T) − φ_eq(T)
with a diverging RdBu colormap centered white at Δ = 0.
Output: ../panels_paper/heatmap_LSE_smart_v20_cpu_N<N>_residual.{png,pdf}
=#
...
const ALPHA_MAX = length(ARGS) >= 2 ? parse(Float64, ARGS[2]) : 0.60
...
csv_in = joinpath(@__DIR__, @sprintf("basin_stab_LSE_smart_v20_cpu_N%d.csv", N_ARG))
isfile(csv_in) || error("Missing $csv_in — run basin_stab_LSE_smart_v20_cpu.jl first")
...
p1 = heatmap(alphas, Ts, res_grid,
    ...
    title=@sprintf("Smart-MC LSE v20 (CPU)  N=%d  K=10⁴  sampled C", N_ARG),
...
for ext in ("png", "pdf")
    savefig(p1, joinpath(out_dir,
        @sprintf("heatmap_LSE_smart_v20_cpu_N%d_residual.%s", N_ARG, ext)))
end
@printf("Saved: panels_paper/heatmap_LSE_smart_v20_cpu_N%d_residual.{png,pdf}\n", N_ARG)
