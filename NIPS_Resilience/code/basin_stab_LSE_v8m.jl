#=
GPU-Accelerated LSE Basin Stability Test (v8m) — PER-SAMPLE OUTPUT
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSE_v8m.jl           # resume from last completed α
  julia basin_stab_LSE_v8m.jl --fresh   # overwrite CSV, start from scratch

Goal: Test whether the Gaussian or exact-distribution phase boundary
  is correct for LSE.  Per-sample output enables histogram analysis.

  Per disorder sample, saves: φ_a, φ_b, q_12, φ_max_other
  Single CSV output: basin_stab_LSE_v8m.csv
...
function main()
    ...
    println("=" ^ 70)
    println("LSE Basin Stability Test – GPU v8m (β_net=$(betanet), per-sample output)")
    ...
    csv_out = "basin_stab_LSE_v8m.csv"
