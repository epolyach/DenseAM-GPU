#=
Smart-MC LSE heatmap (v19 — dynamic regeneration) — AAAI paper
────────────────────────────────────────────────────────────────────────
Reads basin_stab_LSE_smart_v19_N<N>_K<K>.csv and produces a (α, T)
...
Usage:
  julia plot_LSE_smart_v19_heatmap.jl N K
  (defaults: N=500, K=10000)

Output: ../panels_paper/heatmap_LSE_smart_v19_N<N>_K<K>.{png,pdf}
────────────────────────────────────────────────────────────────────────
=#
...
csv_in = joinpath(@__DIR__,
    @sprintf("basin_stab_LSE_smart_v19_N%d_K%d.csv", N_ARG, K_ARG))
isfile(csv_in) || error("Missing $csv_in — run basin_stab_LSE_smart_v19_dynamic.jl first")
...
# Per-α columns onto a dense T-axis (wedge → uniform via linear interp).
na = length(alphas)
...
p1 = heatmap(alphas, Ts, phi_grid,
    ...
    colorbar_title="⟨φ⟩",
    title=@sprintf("Smart-MC LSE v19 (dynamic)   N=%d  K=%d", N_ARG, K_ARG),
...
for ext in ("png", "pdf")
    savefig(p1, joinpath(out_dir,
        @sprintf("heatmap_LSE_smart_v19_N%d_K%d.%s", N_ARG, K_ARG, ext)))
end
@printf("Saved: panels_paper/heatmap_LSE_smart_v19_N%d_K%d.{png,pdf}\n",
        N_ARG, K_ARG)
println("\nDone.")
