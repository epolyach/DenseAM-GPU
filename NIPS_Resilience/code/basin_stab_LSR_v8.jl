#=
GPU-Accelerated LSR Basin Stability Test (v8) — PRODUCTION
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSR_v8.jl           # resume from last completed α
  julia basin_stab_LSR_v8.jl --fresh   # overwrite CSVs, start from α=0.20

v8 changes vs v7:
  - Added φ_max_other: maximum overlap with any non-target pattern
    φ_max_other(α,T) = ⟨max_{μ≠1} x·ξ^μ/N⟩  (from replica A)
    This detects mixture states where the system has significant overlap
    with spurious patterns.
  - α range narrowed to [0.20, 0.35] (focus on α_th region)
  - α step refined to 0.005
  - Three CSV outputs: φ, q, φ_max_other
...
- Sampling: N_SAMP = 2^12 = 4096 steps (φ, q, φ_max_other measured)
...
- Output: basin_stab_LSR_v8.csv       (φ map)
          basin_stab_LSR_v8_q.csv      (q map)
          basin_stab_LSR_v8_phimax.csv (φ_max_other map)

Resume: by default, reads existing CSV and continues from the first
    missing α row. All three CSVs are checked; uses min(rows) for safety.
...
function main()
    ...
    println("=" ^ 70)
    println("LSR Basin Stability Test – GPU v8 (b=$(round(b_lsr, digits=3)), two-replica + φ_max_other)")
    println("  v8: φ + q_EA + φ_max_other maps")
    ...
    csv_phi    = "basin_stab_LSR_v8.csv"
    csv_q      = "basin_stab_LSR_v8_q.csv"
    csv_phimax = "basin_stab_LSR_v8_phimax.csv"
    n_completed = FRESH_START ? 0 : minimum([count_completed_rows(f)
                                              for f in (csv_phi, csv_q, csv_phimax)])
    ...
    if FRESH_START || n_completed == 0
        for csv_file in (csv_phi, csv_q, csv_phimax)
    ...
    phimax_grid = zeros(Float64, n_alpha, n_T)
    t_total_eq = 0.0
    t_total_samp = 0.0
    ...
    for chunk_start in start_idx:chunk_size:n_alpha
        ...
        phimax_g  = Vector{CuVector{F}}(undef, n_chunk)   # v8: φ_max_other accumulator
        ...
        for (local_i, global_i) in enumerate(chunk_indices)
            ...
            phimax_g[local_i] = CUDA.zeros(F, n_chains)   # v8
        ...
        println("Sampling ($N_SAMP steps, measuring φ, q, φ_max_other)...")
        ...
        for (local_i, global_i) in enumerate(chunk_indices)
            ...
            # φ_max_other: from replica A over N_SAMP steps → divide by N_SAMP
            phimax_avg = Array(phimax_g[local_i]) ./ N_SAMP
            phimax_mat = reshape(phimax_avg, n_T, n_dis)
            phimax_grid[global_i, :] = vec(mean(phimax_mat, dims=2))
        end

        # ── Append chunk results to CSVs ──
        for (csv_file, grid) in ((csv_phi, phi_grid), (csv_q, q_grid), (csv_phimax, phimax_grid))
            open(csv_file, "a") do f
                for (local_i, global_i) in enumerate(chunk_indices)
                    write(f, @sprintf("%.3f", alpha_vec[global_i]))
                    for j in 1:n_T
    ...
    println("CSV saved: $csv_phimax (φ_max_other map)")
    println()
