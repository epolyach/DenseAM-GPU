#=
GPU-Accelerated LSR Basin Stability Test (v10) — PHASE BOUNDARY STUDY
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSR_v10.jl           # resume from last completed α
  julia basin_stab_LSR_v10.jl --fresh   # overwrite CSV, start from scratch
...
v8m: Same MC as v8 but saves per-disorder-sample data instead of
  ...
  Single CSV output: basin_stab_LSR_v10.csv
...
- Output: basin_stab_LSR_v10.csv (per-sample: α, T, disorder, φ_a, φ_b, q12, φ_max_other)
...
function main()
    ...
    csv_out = "basin_stab_LSR_v10.csv"
