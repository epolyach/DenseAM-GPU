...
Output:
  ...
  v16b_Pesc_a{α}_T{T}.csv:  survival curves S(t)
────────────────────────────────────────────────────────────────────────
=#
...
const M_PAT             = 20000
const SUMMARY_FILE      = "v16b_summary.csv"
const TRIALS_FILE       = "v16b_trials.csv"
...
# v16b: additional points — high T + below-contour at high α
const ALPHA_VALUES = [0.20, 0.22, 0.24, 0.26, 0.28, 0.30]
const T_VALUES     = [0.60, 0.70, 1.20, 1.60, 2.00]

# Additional below-contour points at α > 0.3
const EXTRA_POINTS = [
    (0.32, 0.20), (0.34, 0.15), (0.32, 0.25),
]
...
function get_run_params(alpha, T)
    if T >= 1.0
        return (10_000, 4)
    elseif T >= 0.60
        return (15_000, 4)   # T=0.6-0.7 — moderate speed
    elseif T >= 0.50
        return (30_000, 8)
    elseif T >= 0.30
        return (100_000, 16)
    elseif T >= 0.20
        if alpha >= 0.30; return (100_000, 16)
...
function run_point!(α, T, T_run, stride, n_dis)
    ...
    pesc_file = @sprintf("v16b_Pesc_a%.2f_T%.2f.csv", α, T)
...
function main()
    ...
    println("=" ^ 70)
    println("v16b — High-T + below-contour extension")
