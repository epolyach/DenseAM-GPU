...
Output: panels_paper/all_survival.{png,pdf}
────────────────────────────────────────────────────────────────────────
=#
...
# ──────────────── Collect all v16/v16b/v17 files (α ≥ 0.20) ────────────────
# Prefer v17 > v16b > v16 where overlapping
v14_dir = @__DIR__
all_pesc = filter(f -> (startswith(f, "v16_Pesc_a") || startswith(f, "v16b_Pesc_a") || startswith(f, "v17_Pesc_a")) && endswith(f, ".csv"), readdir(v14_dir))

# Group by (α, T), keep best version
...
for f in all_pesc
    ...
    α < 0.20 && continue
    ver = startswith(f, "v17") ? "v17" : (startswith(f, "v16b") ? "v16b" : "v16")
    key = (α, T)
    if !haskey(file_map, key) || priority[ver] > priority[split(file_map[key], "_")[1]]
        file_map[key] = f
    end
end
all_files = sort(collect(values(file_map)))
...
fig = plot(plots_arr..., layout=(nT, na),
           size=(na * 250, nT * 200), dpi=200,
           margin=1Plots.mm)

for ext in ("png", "pdf")
    savefig(fig, joinpath(out_dir, "all_survival.$ext"))
end
println("Saved: panels_paper/all_survival.{png,pdf}")
