#=
GPU-Accelerated LSE Basin Stability Test (v10) — PHASE BOUNDARY STUDY
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSE_v10.jl           # resume from last completed α
  julia basin_stab_LSE_v10.jl --fresh   # overwrite CSV, start from scratch

Goal: Test whether the Gaussian or exact-distribution phase boundary
  is correct for LSE.  Lighter than v8m (half steps, smaller P range).
  Same protocol as basin_stab_LSR_v10.jl for direct comparison.

  Per disorder sample, saves: φ_a, φ_b, q_12, φ_max_other
  Single CSV output: basin_stab_LSE_v10.csv

Protocol:
- α: 0.02 to 1.0, step 0.02 (50 values)
- T: 40 values in (0, 1], exponentially concentrated at low T
- M: 10,000 → 200,000 (sub-exponential interpolation)
- N_EQ = 2^14, N_SAMP = 2^12
- n_dis = 256 (fixed for all α)
- Two replicas per disorder sample
- LSE energy with β_net = 1.0
...
const PHI_MAX     = F(1.0)
const N_DIS       = 256               # fixed disorder samples per α
const N_EQ        = 2^14              # 16384 equilibration steps (unmeasured)
const N_SAMP      = 2^12              # 4096 sampling steps (measured)

const alpha_vec = collect(F(0.02):F(0.02):F(1.0))      # 50 values
# Custom T grid: exponentially concentrated at low T (where theories diverge)
# f_i = exp(dw*i) - 1 for i=1:40, T_i = f_i / max(f_i)
const T_vec     = let dw = 0.1; fi = [exp(dw*i) - 1 for i in 1:40]; F.(fi ./ maximum(fi)); end  # 40 values in (0, 1]
const n_alpha   = length(alpha_vec)
const n_T       = length(T_vec)
const MIN_PAT   = 10000
const MAX_PAT   = 200000
const ind       = 10
...
const n_trials_vec   = fill(2 * N_DIS, n_alpha)  # 2 replicas × N_DIS
const n_disorder_vec = fill(N_DIS, n_alpha)
...
function compute_energy_lse!(E::CuVector{F}, x::CuArray{F,3},
                              ...
                              Nf::F)
    # overlap[P, n_T, n_dis] = patterns^T * x  (raw dot products)
    CUDA.CUBLAS.gemm_strided_batched!('T', 'N', one(F), patterns, x, zero(F), overlap)

    # Log-sum-exp with numerical stabilization
    @. overlap = -betanet * (Nf - overlap)   # = βnet * (x·ξ^μ - N) = -βnet*N*(1-φ^μ)
    m = maximum(overlap, dims=1)             # max over patterns for stability
...
function main()
    ...
    println("=" ^ 70)
    println("LSE Basin Stability Test – GPU v10 (β_net=$(betanet), per-sample output)")
    println("  v8m: per-disorder-sample φ_a, φ_b, q_EA, φ_max_other")
    println("  Two-replica cross-overlap for q_EA")
    println("  σ(N,T) = 2.4·T/√N")
    println("=" ^ 70)
    println("Protocol: Retrieval / Basin of Attraction")
    println("  Initial alignment: φ ∈ [$PHI_MIN, $PHI_MAX] (random per replica)")
    println("  LSE kernel: β_net = $(betanet) (global support, no hard wall)")
    ...
    println("  Disorder samples: $N_DIS (fixed for all α)")
    ...
    csv_out = "basin_stab_LSE_v10.csv"
