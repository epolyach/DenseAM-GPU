#=
GPU-Accelerated LSR N-Scaling Test (v12b, additional finite-size probes)
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSR_v12b_finite_size.jl
  julia basin_stab_LSR_v12b_finite_size.jl --fresh
...
Output:
  v12b_trajectory_a{α}_M{M}_T{T}.csv
  v12b_summary.csv
────────────────────────────────────────────────────────────────────────
=#
...
# Additional finite-size probes (2026-07):
# (a) small-M points at alpha=0.20 -> leftward shift of the apparent boundary
# (b) discriminator at alpha=0.28, T=0.50, M up to 2e6:
#     union-bound law predicts frac_escaped DROPS with M;
#     literal Eq.12 K-scaling predicts it RISES.
const PROBE_POINTS = Tuple[
    # (alpha, T, M, n_dis)
    (0.20, 0.25,     2_000, 2000),   # N=38
    (0.20, 0.40,     2_000, 2000),
    (0.20, 0.25,     6_300, 2000),   # N=44
    (0.20, 0.40,     6_300, 2000),
    (0.28, 0.50,    20_000, 2000),   # N=35
    (0.28, 0.50,   200_000,  800),   # N=44
    (0.28, 0.50, 2_000_000, 100),    # N=52
]
...
function main()
    ...
    for (pi, pt) in enumerate(PROBE_POINTS)
        ...
        traj_file = @sprintf("v12b_trajectory_a%.2f_M%d_T%.2f.csv", α, M, T)
