#=
GPU-Accelerated LSR Basin Stability Test (v3)
────────────────────────────────────────────────────────────────────────
...
- Output: basin_stab_LSR_v3.csv

v3 change vs v2: Split equilibration / sampling so φ is NOT polluted
...
const N_EQ        = 2^14              # 2^14 = 16384 equilibration steps (unmeasured)
const N_SAMP      = 2^12              # 2^12 = 4096 sampling steps (measured)
...
const TARGET_MEM_PER_CHUNK_GB = 43.0
...
function main()
    ...
    println("=" ^ 70)
    println("LSR Basin Stability Test – GPU v3 (b=$(round(b_lsr, digits=3)))")
    println("=" ^ 70)
    ...
    # ── Write CSV header ──
    csv_file = "basin_stab_LSR_v3.csv"
    ...
    for chunk_start in 1:chunk_size:n_alpha
        ...
        for step in 1:N_SAMP
            for (local_i, global_i) in enumerate(chunk_indices)
                ...
                mc_step!(xs_g[local_i], xps_g[local_i], Es_g[local_i], Eps_g[local_i],
                         pats_g[local_i], ovs_g[local_i], β_gpus[local_i], ras[local_i],
                         Nf, ssvec[local_i])
                # Accumulate alignment (sampling phase only)
        ...
        for (local_i, global_i) in enumerate(chunk_indices)
            n_trials = n_trials_vec[global_i]
            phi_avg = Array(phis_g[local_i]) ./ N_SAMP     # average over sampling steps only
            phi_mat = reshape(phi_avg, n_T, n_trials)
            phi_grid[global_i, :] = vec(mean(phi_mat, dims=2))  # average over trials
        end
