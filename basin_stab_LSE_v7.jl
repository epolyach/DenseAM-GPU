#=
GPU-Accelerated LSE Basin Stability Test (v7) — PRODUCTION
────────────────────────────────────────────────────────────────────────
Usage:
  julia basin_stab_LSE_v7.jl           # resume from last completed α
  julia basin_stab_LSE_v7.jl --fresh   # overwrite CSVs, start from α=0.01
...
- LSE energy (log-sum-exp, βnet = 1.0)
- Output: basin_stab_LSE_v7.csv  (φ map)
          basin_stab_LSE_v7_q.csv (q map)
...
See also: basin_stab_LSE_v7_test.jl  (same code, reduced parameters)
...
function main()
    ...
    println("=" ^ 70)
    println("LSE Basin Stability Test – GPU v7 (two-replica q_EA)")
    ...
    for (idx, α) in enumerate(alpha_vec)
        ...
        n_chains = n_T * n_dis  # chains per replica
